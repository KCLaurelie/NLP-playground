...
def super_read_csv(file_path,usecols=None,clean_results=True, filter_col=None,filter_value=None, threshold_col=None, threshold_value=None,
    ...
    if os.stat(file_path).st_size/1e9>=1.5: # large csv files need to be read by chunks
        ...
        if read_from_SQL_export: # file exported using SSIS package method
            TextFileReader = pd.read_csv(file_path, sep='|', header=0, encoding='ansi', quoting=1, escapechar='\\', engine='python', usecols=usecols, chunksize=10000)
        else:
            TextFileReader=pd.read_csv(file_path,usecols=usecols,chunksize=10000,header=0,engine='python',error_bad_lines=False)
        ...
        try: #if we're lucky we can load all that in 1 go
            ...
            if clean_results: res=clean_df(res,filter_col=filter_col,filter_value=filter_value, threshold_col=threshold_col, threshold_value=threshold_value)
        except: #if we're less lucky let's concatenate each chunk individually
            res=pd.DataFrame()
            for chunk in TextFileReader:
                if clean_results: chunk=clean_df(chunk,filter_col=filter_col,filter_value=filter_value, threshold_col=threshold_col, threshold_value=threshold_value)
                res=pd.concat([res, chunk],ignore_index=True)
                
    else: #normal sized csv files
        if read_from_SQL_export: # file exported using SSIS package method
            res = pd.read_csv(file_path, sep='|', header=0, encoding='ansi', quoting=1, escapechar='\\', engine='python', usecols=usecols)
        else:
            try: #file exported directly from SQL (not using SSIS export method)
                ...
                res=pd.read_csv(file_path,header=0,usecols=usecols,engine='python',error_bad_lines=False,encoding='ISO-8859-1')
        if clean_results: res=clean_df(res,filter_col=filter_col,filter_value=filter_value, threshold_col=threshold_col, threshold_value=threshold_value)
    return res
...
def load_symptoms_df(symptoms_file,
    ...
    res_df=pd.DataFrame()
    TextFileReader=pd.read_csv(symptoms_file,usecols=usecols,chunksize=10000,header=0,engine='python',error_bad_lines=False)
    for chunk in TextFileReader:
...
def monthlist_short(dates):
    ...
    return OrderedDict(((start + datetime.timedelta(_)).strftime(r"%b-%y"), None) for _ in range((end - start).days)).keys()
...
def convert_to_longitudinal_dates(df,
    ...
    res_cols=[key_col]+list(map(str,range(int(min_date),int(max_date+1)))) #[x for x in df.columns if 'stay' in x or key_col in x]
    res=df[res_cols].groupby([key_col],as_index=False).sum().sort_values(by=[key_col])
    return res
