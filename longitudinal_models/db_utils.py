        ...
        for record in query_res.itertuples():
            if len(record.Attachment_Text)>1: # sometimes sql query doesn't exclude all empty texts...
                ...
                with open(os.path.join(tmp_path,title+'.txt'), "w") as text_file:
                    try: text_file.write(record.Attachment_Text) #sometimes it breaks because of weird characters
                    except: pass #in which case we skip for now (TODO: clean weird stuff in SQL query)
    connection.close()
    return 0
