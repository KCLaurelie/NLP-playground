...
def generate_sample(patients_file=r'T:\aurelie_mascio\F20 corpus\patients_static_data.csv',
                    sample_size=1000,
                    random_state=1,
                    sample_cols=['brcid','len_text','date_of_birth','first_language', 'gender','employment', 'occupation', 'religion','marital_status', 'ethnicity','secondary_diag']):
...
def extract_texts_from_csv(file_path=root_path+r'\multimorbidity\data_multimorbidity.csv',
                           ...
                           usecols=['BrcId','CN_Doc_ID','doc_date','Attachment_Text'], #['BRC_ID','cATCN10','cATDa0', 'cATAt03'],
    ...
    if read_from_SQL_export: # file exported using SSIS package method
        ...
        TextFileReader = pd.read_csv(file_path,usecols=usecols,chunksize=100000,low_memory=False,header=0,encoding='utf8',engine='c',error_bad_lines=False)
...
def extract_sentences_from_CRIS_frontend_output(CRIS_file,
    ...
    TextFileReader=pd.read_csv(CRIS_file,usecols=usecols,chunksize=100000,low_memory=False,header=0,encoding='utf8',engine='c',error_bad_lines=False)
