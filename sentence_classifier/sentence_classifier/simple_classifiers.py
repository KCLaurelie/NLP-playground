...
def prep_classifier_dataset(sentences, labels, emb_model='tfidf', max_features_idf=1000, test_size=0.2, output_dir=None, **kwargs):
...
def train_classifier(sentences, labels, classifier, emb_model='tfidf', SEED=0, test_size=0.2, output_dir=None, **kwargs):
    # split data and prepare model
    X_train_vect, X_test_vect, y_train, y_test = prep_classifier_dataset(sentences=sentences, labels=labels, emb_model=emb_model, test_size=test_size, output_dir=output_dir, **kwargs)
    ...
    print("TRAIN -- F1: {:.3f} Acc: {:.3f} P: {:.3f} R: {:.3f}".format(tr_perf['f1'], tr_perf['acc'], tr_perf['p'], tr_perf['r']))
    ...
    try: # only works for some classifiers
        print('The most important features are:', sorted(zip(model.feature_importances_, X_train_vect.columns), reverse=True)[0:10])
    except:
        pass
    if output_dir is not None:
        try:
