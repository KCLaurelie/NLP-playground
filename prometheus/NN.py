...
def test():
    n_epochs = 1
    datafile = '/home/ZKraljevic/data/covid_anxiety/anxiety_batch_1_riley.xlsx'
    text_col, label_col = 'clean_text', 'status'
    ####################################################################################
    # 0. load pretrained embedding model
    ####################################################################################
    ...
    # convert to roch format
    emb_model_torch = embedding2torch(emb_model, SEED=0)
    embeddings = emb_model_torch['embeddings']
    ####################################################################################
    # 1. Load Dataset
    ####################################################################################
    df = pd.read_excel(datafile)[0:100]
    df[label_col] = df[label_col].fillna('irrelevant').replace({'irrelevant':0, 'affirmed':1, 'negated':0})
    df.head()

    ####################################################################################
    # 2. Train and evaluate the RNN model
    ####################################################################################
    nn_model=RNN(emb_model_torch['embeddings'], padding_idx=emb_model_torch['word2id']['<PAD>'], rnn_type='lstm')
    print(nn_model)
