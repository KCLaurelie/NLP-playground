...
if num_words is None: #use entire vocabulary (do not cut long texts)
...
#add diiferent layers to network: Gated Recurrent Unit (GRU) to the network.
model.add(GRU(units=16, return_sequences=True)) #1st layer uses 16 output. we need to return sequences of data because the next GRU expects sequences as its input.
model.add(GRU(units=8, return_sequences=True))
model.add(GRU(units=4))
model.add(Dense(1, activation='sigmoid'))#fully-connected / dense layer which computes a value between 0.0 and 1.0 that will be used as the classification output
